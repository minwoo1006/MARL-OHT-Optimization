...
def safe_compute_action(algo, obs, env=None, agent_id=None):
    ...
    # ----------------------------------------------------
    # Evaluation-time action masking
    # ----------------------------------------------------
    if env is not None and agent_id is not None:
        ...
        # successor 개수에 따라 action 1, 2 허용
        if len(neighbors) >= 1:
...
def main():
    ...
    # 모든 OHT agent는 동일한 observation/action space를 공유한다.
    ...
    random_2_summary, _ = evaluate_random_policy(
    num_ohts=2,
    max_steps=200,
    num_episodes=20,
    )
    dijkstra_2_summary, _ = evaluate_dijkstra_policy(
        num_ohts=2,
        max_steps=200,
        num_episodes=20,
    )
    ppo_2_summary, _ = evaluate_ppo_policy(
        algo,
        num_ohts=2,
        max_steps=200,
        num_episodes=20,
    ...
    print_comparison_table(
        ...
        title="Policy Comparison: 2 OHTs",
    )
    random_5_summary, _ = evaluate_random_policy(
        num_ohts=5,
        max_steps=200,
        num_episodes=20,
    )
    dijkstra_5_summary, _ = evaluate_dijkstra_policy(
        num_ohts=5,
        max_steps=200,
        num_episodes=20,
    )
    ppo_5_summary, _ = evaluate_ppo_policy(
        algo,
        num_ohts=5,
        max_steps=200,
        num_episodes=5,
    ...
    print_comparison_table(
        ...
        title="Policy Comparison: 5 OHTs",
    )

    random_10_summary, _ = evaluate_random_policy(
        num_ohts=10,
        max_steps=200,
        num_episodes=20,
    )
    dijkstra_10_summary, _ = evaluate_dijkstra_policy(
        num_ohts=10,
        max_steps=200,
        num_episodes=20,
    )
    ppo_10_summary, _ = evaluate_ppo_policy(
        algo,
        num_ohts=10,
        max_steps=200,
        num_episodes=20,
    ...
    print_comparison_table(
        ...
        title="Policy Comparison: 10 OHTs",
    )
