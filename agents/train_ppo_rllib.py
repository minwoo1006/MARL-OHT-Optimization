...
def collect_congestion_data(
    ...
    for episode_idx in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            for agent_id, info in infos.items():
                ...
                active_hot_lots += is_hot_lot

                if stall >= reward_config.get("stall_threshold", 15):
                    deadlock_events += 1
            ...
            hot_lot_active_values.append(active_hot_lots)
    ...
    return {
        ...
        "avg_active_hotlot_count": float(np.mean(hot_lot_active_values)) if hot_lot_active_values else 0.0,
...
def main():
    ...
    congestion_log_interval = int(os.environ.get("OHT_CONGESTION_LOG_INTERVAL", "10"))
    congestion_eval_episodes = int(os.environ.get("OHT_CONGESTION_EVAL_EPISODES", "3"))
    congestion_eval_ohts = int(os.environ.get("OHT_CONGESTION_EVAL_OHTS", str(train_num_ohts)))
    congestion_csv_path = os.environ.get("OHT_CONGESTION_CSV", "logs/congestion_data.csv")
    ...
    logger.init(config={
        ...
        "congestion_log_interval": congestion_log_interval,
        "congestion_eval_episodes": congestion_eval_episodes,
        "congestion_eval_ohts": congestion_eval_ohts,
        "congestion_csv_path": congestion_csv_path,
    })
    ...
    for i in range(num_iterations):
        ...
        if congestion_log_interval > 0 and (
            (i + 1) % congestion_log_interval == 0 or i == 0
        ):
            congestion_data = collect_congestion_data(
                algo,
                iteration=i,
                num_ohts=congestion_eval_ohts,
                max_steps=env_config["max_steps"],
                num_episodes=congestion_eval_episodes,
                csv_path=congestion_csv_path,
                map_config=map_config,
                reward_config=reward_config,
                hot_lot_probability=hot_lot_probability,
            )
